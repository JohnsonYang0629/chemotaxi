...
def plot_trajectories(trajectories, output_filename, dt=1.0, colormaps=None, center=None, plot_range=None,
                      solid_color=False, time_period=None):
    """
    ...
    Filters the trajectories if a time_period (t1, t2) is provided.
    """
    if not trajectories:
        print("No trajectory data to plot.")
        return

    # Filter trajectories based on time_period
    plot_dict = {}
    for pid, traj in trajectories.items():
        if time_period:
            t1, t2 = time_period
            start_idx = max(0, int(t1 / dt))
            end_idx = min(len(traj), int(t2 / dt) + 1)
        else:
            start_idx = 0
            end_idx = len(traj)

        filtered_traj = traj[start_idx:end_idx]

        # We need at least 2 points to draw a line segment
        if len(filtered_traj) >= 2:
            plot_dict[pid] = {
                'data': filtered_traj,
                'times': np.arange(start_idx, start_idx + len(filtered_traj)) * dt
            }

    if not plot_dict:
        print("No valid trajectory points found within the specified time period.")
        return

    num_particles = len(plot_dict)
    first_pid = next(iter(plot_dict))
    is_3d = plot_dict[first_pid]['data'].shape[1] > 4
    ...
    if is_3d:
        ...
        title_suffix = f" (t={time_period[0]} to {time_period[1]})" if time_period else ""
        ax.set_title(f'{num_particles} Particle Trajectories (3D){title_suffix}')
        ...
        title_suffix = f" (t={time_period[0]} to {time_period[1]})" if time_period else ""
        ax.set_title(f'{num_particles} Particle Trajectories (2D){title_suffix}')
        ax.set_xlabel('X Coordinate')
        ax.set_ylabel('Y Coordinate')
    ...
    for i, (pid, p_info) in enumerate(plot_dict.items()):
        cmap = colormaps[i % len(colormaps)]
        traj_data = p_info['data']
        time_values = p_info['times']

        plot_data = traj_data[:, :3] if is_3d else traj_data[:, :2]
        ...
        # ==== 替换部分：起点统一改为绿色圆点，终点为红色圆点 ====
        if is_3d:
            p_start = ax.scatter(plot_data[0, 0], plot_data[0, 1], plot_data[0, 2],
                                 color='green', s=100, marker='o', edgecolors='black', depthshade=False, zorder=5)
            p_end = ax.scatter(plot_data[-1, 0], plot_data[-1, 1], plot_data[-1, 2],
                               color='red', s=100, marker='o', edgecolors='black', depthshade=False, zorder=5)
        else:
            p_start = ax.scatter(plot_data[0, 0], plot_data[0, 1],
                                 color='green', s=100, marker='o', edgecolors='black', zorder=5)
    ...
    legend_labels = []
    for pid in plot_dict.keys():
        legend_labels.append(f'Particle {pid} Start')
        legend_labels.append(f'Particle {pid} End')
    ...
    if center is not None and plot_range is not None:
        ...
        # Only use the points currently being plotted for automatic bounding box bounds
        all_coords = np.vstack([p['data'] for p in plot_dict.values()])
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Examples:\n"
               ...
               "2. Plot specific time range: python visualize_trajectory.py results/run.config --time_period 10.0 50.0\n"
               "3. Select specific colormaps: python visualize_trajectory.py results/run.config --cmap_indices 2 4 8\n"
    ...
    parser.add_argument("--time_period", type=float, nargs=2, metavar=('t1', 't2'),
                        help="Specify the time period to plot (t1 t2). Only data points within this time range will be drawn.")
    ...
    # Modify the output filename if time_period is specified
    if args.time_period:
        output_filename = f"{base_name}_t{args.time_period[0]}_to_{args.time_period[1]}.trajectories.png"
    else:
