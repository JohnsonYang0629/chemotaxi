...
from matplotlib.colors import TwoSlopeNorm, CenteredNorm, LinearSegmentedColormap
...
# --- Custom Colormap ---
# Blue (-) -> Black (0) -> Red (+)
bbr_colors = ["#2166ac", "black", "#b2182b"]
bbr_cmap = LinearSegmentedColormap.from_list("blue_black_red", bbr_colors)
...
def _precompute_rotated_nodes_2d(n_history_steps, n_nodes, trajectory_history, nodes_body_frame):
    ...
    for i in prange(n_history_steps):
        ...
        # orient_x, orient_y from trajectory
...
def load_particle_structures(sim_params):
    """Loads particle vertex structures, chemical distributions, and mobility distributions."""
...
def plot_3d_sliced_view(yz_slice_data, xz_slice_data, grid_x, grid_y, grid_z, x_slice_idx, y_slice_idx, trajectories,
    ...
    if max_abs == 0: max_abs = 1e-6
    norm = CenteredNorm(vcenter=0.0, halfrange=max_abs)
    ...
    if sim_params.domain == '2D':
        ...
        for i in range(trajectories.shape[0]):
            ...
            if args.visualize_particle != 'none':
                ...
                circle = patches.Circle((current_x, current_y), radius, color='white', alpha=0.5, zorder=4)
                ax.add_patch(circle)
                ...
                if args.visualize_particle in ['chem', 'mobility', 'all']:
                    ...
                    # 小圆点半径设定为大颗粒半径的 10%，它在data坐标系里会随坐标轴一起原比例缩放
                    marker_radius = radius * 0.1

                    if args.visualize_particle in ['chem', 'all']:
                        vals = surface_sources_list[i] * len(nodes)
                        world_nodes = np.array([current_x, current_y]) + (nodes[:, :2] * 0.9) @ rot_mat.T

                        cmap = plt.get_cmap('PiYG_r')
                        if np.max(vals) == np.min(vals):
                            norm_dist = Normalize(vmin=vals[0] - 0.1, vmax=vals[0] + 0.1)
                        else:
                            norm_dist = CenteredNorm(vcenter=0.0)

                        for j in range(len(world_nodes)):
                            color = cmap(norm_dist(vals[j]))
                            dot = patches.Circle(world_nodes[j], radius=marker_radius,
                                                 facecolor=color, edgecolor='none', alpha=0.8, zorder=6)
                            ax.add_patch(dot)

                    if args.visualize_particle in ['mobility', 'all']:
                        vals = mobility_sources_list[i]
                        world_nodes = np.array([current_x, current_y]) + (nodes[:, :2] * 0.7) @ rot_mat.T

                        cmap = plt.get_cmap('plasma')
                        if np.max(vals) == np.min(vals):
                            norm_dist = Normalize(vmin=vals[0] - 0.1, vmax=vals[0] + 0.1)
                        else:
                            norm_dist = Normalize(vmin=np.min(vals), vmax=np.max(vals))
        ...
        if by_label:  # 如果图示非空才绘制 Legend
            ax.legend(by_label.values(), by_label.keys(), facecolor='white', framealpha=0.8)
...
def run_animation_mode(args, sim_params, trajectories, grid_x, grid_y, grid_z):
    ...
    norm = CenteredNorm(vcenter=0.0, halfrange=max_abs)
    print(f"Global symmetric color range set to: [{-max_abs:.4f}, {max_abs:.4f}]")
    ...
    if args.visualize_particle != 'none':
        for i in range(trajectories.shape[0]):
            nodes = nodes_body_frames_list[i]
            radius = np.max(np.sqrt(np.sum(nodes[:, :2] ** 2, axis=1)))
            circle = patches.Circle((trajectories[i, 0, 0], trajectories[i, 0, 1]), radius, color='white', alpha=0.5,
                                    zorder=4)
            ax.add_patch(circle)
            ...
            if args.visualize_particle in ['chem', 'all']:
                vals = surface_sources_list[i] * len(nodes)
                world_nodes = np.array([start_x, start_y]) + (nodes[:, :2] * 0.9) @ rot_mat.T
                ...
                for j in range(len(world_nodes)):
                    ...
                    dot = patches.Circle(world_nodes[j], radius=marker_radius,
                                         facecolor=color, edgecolor='none', alpha=0.8, zorder=6)
                    ax.add_patch(dot)
                    chem_dots.append(dot)
                particle_chem_dots.append(chem_dots)
            else:
                particle_chem_dots.append(None)

            if args.visualize_particle in ['mobility', 'all']:
                vals = mobility_sources_list[i]
                world_nodes = np.array([start_x, start_y]) + (nodes[:, :2] * 0.7) @ rot_mat.T
                ...
                for j in range(len(world_nodes)):
                    ...
                    ax.add_patch(dot)
                    mob_dots.append(dot)
                particle_mob_dots.append(mob_dots)
            else:
            ...
            if args.visualize_particle != 'none':
                ...
                if args.visualize_particle in ['chem', 'mobility', 'all']:
                    ...
                    if particle_chem_dots[i] is not None:
                        world_nodes = np.array([current_x, current_y]) + (nodes[:, :2] * 0.9) @ rot_mat.T
                        chem_dots = particle_chem_dots[i]
                        for j in range(len(world_nodes)):
                            chem_dots[j].center = world_nodes[j]
                            render_objs.append(chem_dots[j])

                    if particle_mob_dots[i] is not None:
                        world_nodes = np.array([current_x, current_y]) + (nodes[:, :2] * 0.7) @ rot_mat.T
                        mob_dots = particle_mob_dots[i]
                        for j in range(len(world_nodes)):
                            mob_dots[j].center = world_nodes[j]
                            render_objs.append(mob_dots[j])
